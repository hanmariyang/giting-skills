...
- **별칭 사전**: 사람이 실제로 하는 말("화면 가운데 뜨는 창", "옆에서 미끄러져 나오는 메뉴") → 정식 명칭 매핑 20종
- **요청 문장**: 각 컴포넌트마다 AI에게 그대로 복사해 쓰는 요청 문장
...
설치하면 Claude가 세 가지를 합니다.

1. **통역** — "위에 눌러서 화면 바뀌는 거" → "그건 탭(Tabs)입니다" + 다음부터 쓸 요청 문장
2. **구현** — 검증된 레퍼런스 HTML을 기반으로 기존 코드 스타일에 맞춰 이식
3. **제안** — "이 목록이 너무 길어" → 상황에 맞는 컴포넌트를 근거와 함께 추천
...
사전 정본(SSOT)은 `plugins/ui-menu/menu.json`, 레퍼런스 구현은 `plugins/ui-menu/components/*.html` 입니다. 갤러리·llms.txt는 전부 여기서 생성됩니다.
